...
use crate::lob::order::{OrderType,OrderSide};
...
impl TypeSelector for UniformTypeSelector {
	fn sample(&mut self, rng: &mut impl Rng) -> (OrderSide, OrderType) {
		...
		if sample < self.new_limit_cutoff {
			(order_side, OrderType::LimitOrder { qty: 0, price: 0 })		
		} else if sample < self.new_market_cutoff {
			(order_side, OrderType::MarketOrder { qty: 0 })		
		} else if sample < self.cancel_cutoff {
			(order_side, OrderType::CancelOrder)		
		} else {
			(order_side, OrderType::UpdateOrder { old_id: 0, qty: 0, price: 0 })		
		}
	} 
}
...
mod tests {
	...
	fn side_selection_ratio_approximately_equals_rate() {
		...
		for _ in 0..1000000 {
			match type_selector.sample(&mut seeded_rng) {
				(OrderSide::Ask, _) => { ask_count += 1; },
				(OrderSide::Bid, _) => { bid_count += 1; },
			}
		} 
	...
	fn type_selection_ratio_approximately_equals_rate() {
		...
		let mut type_selector = UniformTypeSelector::new(0.75, limit_rate, market_rate, cancel_rate, update_rate);
		let mut seeded_rng = ChaCha8Rng::seed_from_u64(1);
		...
		for _ in 0..sample_count {
			match type_selector.sample(&mut seeded_rng) {
				(_, OrderType::LimitOrder { qty: _, price: _ }) => { limit_count += 1; },
				(_, OrderType::MarketOrder { qty: _ }) => { market_count += 1; },
				(_, OrderType::CancelOrder) => { cancel_count += 1; },
				(_, OrderType::UpdateOrder { old_id: _, qty: _, price: _ } ) => { update_count += 1; },
			}
		} 
		const PRECISION: f64 = 0.025;  // NOTE: Picked arbitrarily, lower precision as tradeoff for smaller sample/faster test
