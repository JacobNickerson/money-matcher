...
impl OrderBook {
    ...
    pub fn process_order(&mut self, order: Order) -> Option<LimitOrder> {
        ...
        match order.kind {
            OrderType::Limit { qty, price } => {
                Some(self.add_order(LimitOrder::new(order, qty, price)))
            }
            OrderType::Market { mut qty } => {
                self.market_order(&mut qty, order.side);
                Some(LimitOrder::new(order, qty, 0))
            }
            OrderType::Cancel => self.cancel_order(order.order_id),
    ...
    pub fn best_ask(&mut self) -> Option<Price> {
        ...
        for i in (0..deleted_count).rev() {
            self.ask_orders.remove(&to_delete[i]);
        }
        best
    }
    ...
    fn add_order(&mut self, original_order: LimitOrder) -> LimitOrder {
        ...
        let level = match order.side {
            OrderSide::Bid => self.bid_orders.entry(order.price).or_default(),
            OrderSide::Ask => self.ask_orders.entry(order.price).or_default(),
        };
        level.push(&order);
    ...
    fn match_order(&mut self, order: &mut LimitOrder) {
        match order.side {
            OrderSide::Ask => {
                ...
                for (price, level) in self.bid_orders.iter_mut().rev() {
                    if order.qty == 0 || *price < order.price {
                        break;
                    }
